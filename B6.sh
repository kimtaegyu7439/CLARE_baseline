#!/usr/bin/env bash
#
# B6 — 자기 롤아웃 앵커, 질의점 N 확대 (B6 와 N 만 다름)
...
mkdir -p "${HERE}/results/B6"
LOG="${HERE}/results/B6/train.log"
echo "══ B6  gpu=${CUDA_VISIBLE_DEVICES}  extra=${EXTRA[*]:-none}  $(date '+%F %T %Z')" | tee -a "${LOG}"
python "${HERE}/B6.py" "${EXTRA[@]}" 2>&1 | tee -a "${LOG}"
