#!/usr/bin/env bash
#
# B7 — Fresh Inversion Anchoring (역적분으로 앵커 좌표를 매 스텝 소환)
...
mkdir -p "${HERE}/results/B7"
LOG="${HERE}/results/B7/train.log"
echo "══ B7  gpu=${CUDA_VISIBLE_DEVICES}  extra=${EXTRA[*]:-none}  $(date '+%F %T %Z')" | tee -a "${LOG}"
python "${HERE}/B7.py" "${EXTRA[@]}" 2>&1 | tee -a "${LOG}"
