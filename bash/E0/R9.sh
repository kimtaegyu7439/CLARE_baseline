...
#   R9_full.method.md의 고정물 해시(x₀ · a_tgt · obs)를 R8 로그와 대조해라.
#
...
R9_PY=./lerobot_lsy/src/lerobot/scripts/R9.py
...
if [ -n "${GPU:-}" ]; then
    ...
    echo "[R9] GPU ${GPU} (직접 지정)"
else
    waited=0
    while :; do
        read -r g free <<< "$(pick_gpu)"
        if [ -z "${g}" ]; then
            echo "[R9] nvidia-smi를 못 읽었다. GPU=<idx> 로 직접 지정해라."; exit 1
        fi
        if [ "${free}" -ge "${NEED_MB}" ]; then
            export CUDA_VISIBLE_DEVICES="${g}"
            echo "[R9] GPU ${g} 선택 (여유 ${free} MiB / 필요 ${NEED_MB} MiB, 대기 ${waited}초)"
            break
        fi
        [ "${waited}" = "0" ] && nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu \
            --format=csv 2>/dev/null | sed 's/^/[R9]   /'
        echo "[R9] 여유 ${free} MiB < ${NEED_MB} MiB — ${POLL_SEC}초 뒤 다시 본다 (누적 ${waited}초)"
...
CL_CKPT=${CKPT_ROOT}/task_${TASK_B}/checkpoints/last/pretrained_model
PRETRAIN_CKPT=${PRETRAIN_CKPT:-${PRETRAIN_PATH}}
JOINT_DIR=${JOINT_DIR:-./outputs/R9_joint/${BENCH}/seed_${SEED}/task${TASK_A}_${TASK_B}}
...
# ── 출력 ──────────────────────────────────────────────────────────────────────
OUT_ROOT=${OUT_ROOT:-./outputs/R9}
RUN_TAG=${RUN_TAG:-${BENCH}_seed${SEED}_${ARM}_task${TASK_A}v${TASK_B}}
...
if [ ! -d "${PRETRAIN_CKPT}" ]; then
    echo "[R9] 사전학습 체크포인트가 없다: ${PRETRAIN_CKPT}"
...
if want_model cl && [ ! -d "${CL_CKPT}" ]; then
    if [ "${AUTO_TRAIN}" != "1" ]; then
        echo "[R9] cl 체크포인트가 없다: ${CL_CKPT}   (AUTO_TRAIN=0 이라 멈춘다)"; exit 1
    fi
    echo ""
    echo "══ [R9] cl 학습: E0 lam0 (순차 파인튜닝) task 0..${TASK_B}"
    ...
    CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
        bash "${E0_SH}" || { echo "[R9] E0(lam0) 학습 실패"; exit 1; }
    [ -d "${CL_CKPT}" ] || { echo "[R9] E0가 끝났는데 체크포인트가 없다: ${CL_CKPT}"; exit 1; }
fi

# ── joint: 두 태스크를 균형 배치로 섞어 한 번에 학습 (통제군) ────────────────
if want_model joint && [ ! -d "${JOINT_CKPT}" ]; then
    if [ "${AUTO_TRAIN}" != "1" ]; then
        echo "[R9] joint 체크포인트가 없다: ${JOINT_CKPT}   (AUTO_TRAIN=0 이라 멈춘다)"
    ...
    if [ -d "${JOINT_DIR}" ]; then
        if [ "${REDO_INCOMPLETE:-1}" = "1" ]; then
            echo "[R9] 미완료 joint 잔해 제거: ${JOINT_DIR}"; rm -rf "${JOINT_DIR}"
        else
            echo "[R9] 미완료 joint 잔해가 있다 (REDO_INCOMPLETE=0): ${JOINT_DIR}"; exit 1
        fi
    fi
    echo ""
    echo "══ [R9] joint 학습: task ${TASK_A} + task ${TASK_B} 동시 (통제군)"
    ...
    "${PYTHON}" "${R7_PY}" \
        ...
        --eval_freq=0 \
        --wandb.enable="${WANDB}" \
        || { echo "[R9] joint 학습 실패"; exit 1; }
    [ -d "${JOINT_CKPT}" ] || { echo "[R9] joint가 끝났는데 체크포인트가 없다: ${JOINT_CKPT}"; exit 1; }
fi
...
for spec in "pretrain:${PRETRAIN_CKPT}" "joint:${JOINT_CKPT}" "cl:${CL_CKPT}"; do
    ...
    [ -d "${path}" ] || { echo "[R9] ${key} 체크포인트가 없다: ${path}"; exit 1; }
done
...
echo ""
echo "══ [R9] ${BENCH} seed ${SEED} ${ARM}  ·  c₀ = task ${TASK_A} vs c₁ = task ${TASK_B}"
...
"${PYTHON}" "${R9_PY}" \
    ...
    --run_tag="${RUN_TAG}" \
    --no_plot=true \
    "${extra[@]}" || { echo "[R9] 프로브 실패"; exit 1; }

# ═════════════════════════════════════════════════════════════════════════════
#  [3] 그림 — PLOT_ONLY=1 경로와 정확히 같은 코드로 그린다 (R3.sh와 같은 이유)
# ═════════════════════════════════════════════════════════════════════════════
if compgen -G "${RUN_DIR}/R9_*.npz" > /dev/null; then
    "${PYTHON}" "${R9_PY}" --plot_only --run_dir="${RUN_DIR}" --per_step="${PER_STEP}"
else
    echo "[R9] npz 캐시가 없다: ${RUN_DIR}"; exit 1
fi

echo ""
echo "→ ${RUN_DIR}/R9_full.png / .pdf        요약: 물리시간 평균 히트맵(a~f) + 단면 g~p"
echo "                                        + 패널 q = 에피소드별 롤아웃 길이 타임라인"
echo "→ ${RUN_DIR}/R9_full_step0000.png ...  rollout step마다 한 장 (최대 9장)"
echo "→ ${RUN_DIR}/R9_full.npz              cos · M · cos_shuffle · 표본 수, (S, L, T)"
echo "→ ${RUN_DIR}/R9_full.summary.json     모델별 집계 + 스텝별 routing 블록 수 + 에피소드 길이"
echo "→ ${RUN_DIR}/R9_full.method.md        Δ 추출 방식 · hook 지점 · 시드 · 고정물 해시"
echo ""
