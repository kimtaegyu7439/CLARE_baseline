...
"""R9_A — R9와 같은 측정을, **더 많이 잊은** CL 체크포인트에서 다시.

왜 따로 두는가
    R9는 task0 -> task1 두 단계만 배운 CL을 썼고, 그 모델의 task0 성공률이 0이 아니라
    30%였다(E0 프로브). 즉 "망각했다"고 부르기에 애매한 상태에서 라우팅을 잰 셈이다.
    R9의 결과(joint ≈ CL, 라우팅 안 죽음)가 "라우팅은 원래 안 죽는다"인지 "이 체크포인트가
    덜 잊어서 그렇다"인지 그 실험만으로는 가를 수 없다.

    R9_A는 task0 -> 1 -> 2 -> 3 까지 배운 CL을 쓴다. 이 구간에서는 마지막 태스크를 뺀
    나머지 성공률이 0으로 떨어진다(그래서 "잊었다"가 관측으로 확정된다). 조건은 R9와
    똑같이 task0 vs task1을 쓴다 — 둘 다 이 모델이 이미 잊은 태스크라는 점이 요점이다.

R9와 다른 것은 **어느 체크포인트를 재는가** 하나뿐이다
    측정(Δ, cos, shuffle, 최종 출력단), 프로브 고정물, 관측 규약, 그림 레이아웃은
    R9를 그대로 import해서 쓴다. 두 실험을 나란히 읽으려면 그래야 한다.

    ★ R9는 CL 체크포인트를 stage_ckpt(ckpt_root, task_b)로 잡았다 — 조건 c₁의 태스크와
      CL의 학습 단계가 같은 숫자로 묶여 있었다. R9_A는 그 둘을 가른다:
          --cl_stage=3   CL은 0..3 까지 배운 상태
          --task_a=0 --task_b=1   조건은 task0 vs task1
      cl_stage를 안 주면 R9와 똑같이 task_b가 쓰인다(하위 호환).

사용 예
    bash bash/E0/R9_A.sh
    PLOT_ONLY=1 bash bash/E0/R9_A.sh
"""
...
FINALS = ("512", "7")
FINAL_TITLE = {"512": "Δ before readout (512-d)", "7": "W·Δ after readout (7-d)"}
...
class R9AConfig(R8Config):
    ...
    out_root: str = "outputs/R9_A"

    # ★ CL이 **몇 단계까지** 배웠는가. R9는 이것을 task_b와 같은 값으로 묶어 썼지만,
    #   R9_A는 "0..3까지 배운 모델에 task0 vs task1 조건"을 넣어야 하므로 분리한다.
    #   -1이면 R9와 같은 동작(task_b를 쓴다).
    cl_stage: int = -1
...
def model_specs(cfg) -> list[dict]:
    """R8.model_specs와 같은 표. 단 cl의 학습 단계를 cl_stage로 잡는다.

    R8/R9는 cl = stage_ckpt(ckpt_root, task_b) 였다. 조건 태스크와 CL 단계를 한 숫자로
    묶은 것인데, R9_A는 "0..3을 배운 모델에 task0 vs task1 조건"이라 둘이 다르다.
    """
    a, b = cfg.task_a, cfg.task_b
    stage = cfg.cl_stage if getattr(cfg, "cl_stage", -1) >= 0 else b
    table = {
        "pretrain": {"ckpt": cfg.pretrain_ckpt, "title": "pretrained  (before any task)"},
        "ft_a": {"ckpt": str(stage_ckpt(cfg.ckpt_root, a)), "title": f"FT{a}  (task {a} only)"},
        "ft_b": {"ckpt": cfg.ft1_ckpt, "title": f"FT{b}  (task {b} only)"},
        # joint는 CL과 같은 태스크 집합(0..stage)을 **섞어** 배운 통제군이다.
        # 조건 쌍(task_a/task_b)과 무관하므로 제목도 그것으로 쓰면 안 된다.
        "joint": {"ckpt": cfg.joint_ckpt,
                  "title": f"joint  (tasks 0–{stage} mixed)"},
        "cl": {"ckpt": str(stage_ckpt(cfg.ckpt_root, stage)),
               "title": f"CL  (task 0 → … → {stage})"},
    }
    specs = []
    for key in [k.strip() for k in cfg.models.split(",") if k.strip()]:
        if key not in table:
            raise SystemExit(f"[R9_A] 모르는 모델 이름: {key!r} (가능: {list(table)})")
        if not table[key]["ckpt"]:
            raise SystemExit(f"[R9_A] {key} 체크포인트 경로가 비어 있다.")
        specs.append({"key": key, **table[key]})
    return specs


def cache_name(cfg: R9AConfig) -> str:
    """full 모드는 장면이 조건의 일부라 obs_task라는 개념이 없다 (R8과 같은 규약)."""
    return "R9A_full.npz" if cfg.cond_mode == "full" else f"R9A_lang_obs{cfg.obs_task}.npz"
...
class SubBlockTap:
    ...
    def verify_final(self, tol: float = 1e-3) -> float:
        ...
        if err > tol:
            raise AssertionError(
                f"[R9_A] 최종층 항등식이 깨졌다: max rel err {err:.2e} > {tol:.0e}\n"
    ...
    def verify_residual(self, tol: float = 1e-4) -> float:
        ...
        if worst > tol:
            raise AssertionError(
                f"[R9_A] residual 항등식이 깨졌다: max rel err {worst:.2e} > {tol:.0e}\n"
...
def derangement(n: int, seed: int) -> np.ndarray:
    ...
    if n < 2:
        raise SystemExit(f"[R9_A] shuffle 기준선에는 프로브가 2개 이상 필요하다 (num_probe={n})")
...
def make_task_env(cfg: R9AConfig, task: int):
...
def demo_action_seqs(cfg: R9AConfig, task: int, n_ep: int) -> list[np.ndarray]:
...
def _assert_obs_compatible(mine, theirs) -> None:
    ...
    if type(mine) is not type(theirs):
        raise AssertionError(
            f"[R9_A] 관측 형식이 R7.capture_obs와 다르다: {type(mine)} vs {type(theirs)}\n"
            f"  capture_obs_traj의 _clone_obs 반환 형태를 R7에 맞춰라.")
    if isinstance(mine, dict):
        a, b = set(mine), set(theirs)
        if a != b:
            raise AssertionError(
                f"[R9_A] 관측 키가 R7.capture_obs와 다르다.\n"
                f"  여기만 있는 키: {sorted(a - b)}\n  R7에만 있는 키: {sorted(b - a)}")


@torch.no_grad()
def capture_obs_traj(cfg: R9AConfig, task: int, n_ep: int, steps: np.ndarray,
    ...
    if demos is not None and len(demos) < n_ep:
        raise SystemExit(
            f"[R9_A] task {task}의 데모가 {len(demos)}개뿐인데 num_obs={n_ep}을 요구했다.\n"
    ...
    logging.info(f"[R9_A] task {task}: 관측 {n_ep}ep × {len(steps)}step = {n_ep * len(steps)}개  "
                 f"(alive {n_alive} · held {int((status == HELD).sum())} · "
                 f"frozen {int((status == FROZEN).sum())})")
    for it in info:                               # 에피소드마다 "언제 끝났는가"
        logging.info(f"[R9_A]   task {task} ep {it['episode']}: live {it['live_len']} steps  "
                     f"(demo {it['demo_len']} · env end {it['env_end']}) — {it['reason']}")
    if n_alive < 0.5 * status.size:
        logging.warning(colored(
            f"[R9_A] task {task}: 캡처 지점의 절반 이상이 롤아웃 종료 뒤다. rollout_steps="
...
#  ── 무엇을 재는가 ──────────────────────────────────────────────────────
#
#  "조건부 라우팅(conditional routing)이 망각과 함께 죽는가"를 잰다.
#
#  velocity_net의 각 블록은 조건 c(장면 관측 + 언어 지시문)를 AdaLN-Zero로 받아
#      h ← h + α_attn(c) ⊙ Attn(AdaLN(h; c))      ← 이 항이 Δ_attn
#      h ← h + α_mlp(c)  ⊙ MLP (AdaLN(h; c))      ← 이 항이 Δ_mlp
#  로 자기 표현을 갱신한다. 서로 다른 두 조건 c₀(task_a) / c₁(task_b)를 **같은 x_t**에
#  넣었을 때
#      Δ(c₀)와 Δ(c₁)이 다른 방향  → 그 블록은 조건에 따라 계산 경로를 바꾼다 (라우팅 살아있음)
#      Δ(c₀)와 Δ(c₁)이 같은 방향  → 조건을 사실상 무시한다               (라우팅 붕괴)
#  이므로 지표는 cos(Δ(c₀), Δ(c₁))이다. ★ **cos이 낮을수록 라우팅이 살아있는 것**이다.
#  직관과 부호가 반대라 그림·로그를 읽을 때 매번 헷갈리는 지점이니 주의.
#
#  ── 세 가지 양 ────────────────────────────────────────────────────────
#
#    cos      = ⟨cos(Δ(c₀), Δ(c₁))⟩       두 조건이 방향을 가르는가
#    shuffle  = ⟨cos(Δ, Δ[perm])⟩         조건을 **고정한 채** 프로브 인덱스만 교란한 각도
#    M        = ⟨½(‖Δ(c₀)‖+‖Δ(c₁)‖)⟩      그 증분이 애초에 크기가 있는가
#
#  shuffle이 없으면 cos을 해석할 수 없다. 고차원 표현은 층마다 고유한 anisotropy가 있어
#  "서로 무관한 두 입력"조차 cos이 층마다 0.1이기도 0.7이기도 하다. 그래서 실제로 읽는
#  값은 절대 cos이 아니라 **gap = cos − shuffle** 이다. gap ≈ 0이면 "두 조건이 서로 다른
#  프로브만큼도 안 갈렸다" = 라우팅이 없다는 뜻이고, gap이 크게 음수면 조건 대비가 무작위
#  대비보다도 방향을 더 벌렸다는 뜻이다.
#  M이 필요한 이유는 §5-6과 같다. AdaLN-Zero라 게이트가 죽으면 Δ가 정확히 0이 되는데,
#  거의 0인 벡터의 각도는 노이즈다. 크기 없는 각도를 라우팅으로 세면 안 된다 —
#  그림 캡션의 "라우팅 블록 수"가 gap과 M 두 조건을 **모두** 요구하는 것이 이 때문이다
#  (route_gap_thresh / route_mag_frac).
#
#  ── 격자 ────────────────────────────────────────────────────────────────
#
#      모델(specs) × 에피소드(oi) × 물리시간(si) × flow time(ti) × 조건(ci ∈ {0,1})
#
#  안쪽 두 축이 "네트워크의 어느 지점을 찌르는가"이고 바깥 두 축이 "어떤 상황에서 찌르는가"다.
#    · flow time t   : x_t = (1−t)·x₀ + t·a. t=0은 순수 노이즈, t=1은 데모 청크.
#                      라우팅이 궤적 초반에만 사는지 끝까지 사는지를 가른다.
#    · 물리시간 step : 롤아웃 0..rollout_steps. 라우팅 붕괴가 초기 상태에서만인지
#                      궤적 내내인지를 가른다 (R8에는 없던 축. R8은 step 0 한 점만 봤다).
#  forward 횟수 = |specs| × num_obs × n_step × t_steps × 2. 기본값이면 R8의 n_step배다.
#
#  ── 결과를 어떻게 읽나 ────────────────────────────────────────────────
#
#  가설은 "잊은 모델은 조건을 구분하지 않는다"(cos이 1에 붙고 gap ≈ 0)이다. 그런데
#  **라우팅은 살아있는데 성공률이 0**으로 나오면 그것 자체가 결과다 — 망각이 라우팅
#  붕괴가 아니라 그 아래 어딘가에서 일어난다는 뜻이다. 최종 출력단을 512와 7 두 지점에서
#  따로 재는 것이 그 "어딘가"의 첫 후보를 가르기 위해서다:
#      final512에서 갈렸는데 final7에서 정렬  → 512→7 사영의 null space가 라우팅을 버렸다
#      둘 다 갈렸는데 행동은 같다              → 방향이 아니라 크기/부호 쪽 문제다
#
# ═════════════════════════════════════════════════════════════════════════════
@torch.no_grad()
def run_probe(cfg: R9AConfig, run_dir: Path) -> Path:
    ...
    pol_cfg = ref.config
    # ★ **첫 모델의 정규화 통계를 전 모델 공통 좌표계로 못박는다.** 아래 루프에서 모델마다
    #   assert_shared_norm으로 같은지 확인한다(§5-4). 정규화가 모델마다 다르면 같은 데모
    #   청크가 다른 a_tgt가 되고, 따라서 x_t도 달라져 "같은 지점을 찔렀다"가 깨진다.
    stats = norm_stats(ref)
    horizon = int(pol_cfg.horizon)
    # 청크 16스텝 중 실제로 로봇에 나가는 구간만 남기는 토큰 슬라이스. 아래 모든
    # pair_stats 호출이 이걸 받는다 — 버려지는 뒷부분까지 넣으면 "하지도 않는 행동"으로
    # 라우팅을 판정하게 된다 (R7/R8과 같은 규약).
    e0, e1 = exec_range(pol_cfg, cfg.exec_slice)
    tok = slice(e0, e1)
    logging.info(colored(f"[R9_A] 실행 구간 = 청크 index {e0}..{e1 - 1} 토큰만 잰다", "green"))

    text = {0: task_text(cfg.dataset_prefix, a), 1: task_text(cfg.dataset_prefix, b)}
    logging.info(colored(f"[R9_A] c₀ = {text[0]!r}", "cyan"))
    logging.info(colored(f"[R9_A] c₁ = {text[1]!r}", "cyan"))

    # ── [1] 프로브 고정물 ────────────────────────────────────────────────────
    # "프로브"는 **네트워크를 어느 점에서 찔러볼 것인가**이다. flow matching이므로 궤적
    # 위의 점은 x_t = (1−t)·x₀ + t·a 로 만들어진다. a_tgt는 두 태스크의 전문가 데모 청크를
    # 섞은 풀에서 뽑은 도착점, x₀는 거기 짝지은 가우시안 출발점. 이 둘을 한 번 뽑아 격자
    # 전체에서 재사용하므로, 모델·조건·물리시간이 달라져도 **찌르는 점은 늘 같다**.
    ...
    # shuffle 영점용 교란순열. 프로브 i의 Δ를 프로브 perm[i]의 Δ와 짝지어 "무관한 두
    # 입력의 각도"를 만든다 — 층마다 다른 cos 기준선을 데이터에서 직접 뽑는 셈이다.
    ...
    logging.info(f"[R9_A] 프로브 {cfg.num_probe}개 × t {cfg.t_steps}격자(0..{cfg.t_max})  "
    ...
    if cfg.obs_driver != "demo":
        ...
        logging.info(colored(
            f"[R9_A] 상태열 구동자 = 정책 '{cfg.obs_driver}'. 세 모델이 보는 s는 동일하지만 "
            f"그 상태 분포는 이 모델에 편향된다.", "yellow"))
    else:
        logging.info(colored("[R9_A] 상태열 구동자 = 전문가 데모 재생 (모델 독립)", "green"))
    logging.info(f"[R9_A] 물리시간 {n_step}지점 {list(obs_steps)} × 초기상태 {cfg.num_obs}개")
    ...
    if cfg.cond_mode == "full":
        ...
        logging.info(colored("[R9_A] cond_mode=full — 장면과 지시문을 함께 바꾼다 (본 실험)",
                             "green"))
        ...
        logging.info(colored(f"[R9_A] cond_mode=language — 장면을 task {cfg.obs_task}로 고정 (부록)",
                             "green"))
    else:
    ...
    if not cfg.exclude_dead_obs:
        logging.warning(colored("[R9_A] --exclude_dead_obs=false — 종료된 롤아웃의 정지 장면도 "
                                "평균에 넣는다.", "yellow"))
        pair_used = np.ones_like(pair_alive)
    n_live = pair_used.sum(axis=0)
    logging.info("[R9_A] 물리시간별 유효 에피소드 수: " +
                 "  ".join(f"{int(s)}:{int(c)}/{cfg.num_obs}"
                           for s, c in zip(obs_steps, n_live)))
    if not n_live.any():
        raise SystemExit("[R9_A] 살아있는 (에피소드, 물리시간) 짝이 하나도 없다. "
                         "--rollout_steps 를 줄여라.")
    ...
    logging.info(f"[R9_A] obs 고정물 해시  c₀#{hash_obs[0]}  c₁#{hash_obs[1]}")
    if driver is not None:
        del driver
    del ref
    ...
    for spec in specs:
        ...
        logging.info(colored(f"[R9_A] {spec['key']}: {spec['ckpt']}", "cyan", attrs=["bold"]))
        ...
        for oi in range(cfg.num_obs):
            for si in range(n_step):
                ...
                # ★ t 루프 **바깥**에서 만든다. 조건 벡터는 t에 의존하지 않는데 비전 인코더가
                #   이 루프에서 가장 비싼 부분이라, 안으로 들이면 t_steps배가 그대로 붙는다.
                ...
                for ti, t in enumerate(t_grid):
                    ...
                    d, f = {}, {}
                    for ci in (0, 1):
                        # cond는 (1, D)라 expand로 프로브 N개에 같은 조건을 붙인다 —
                        # 한 forward가 프로브 축을 통째로 배치 처리한다.
                        net(xt, tt, cond[ci].expand(cfg.num_probe, -1))
                        # ★ snapshot()으로 **떠 둬야** 한다. tap.d는 hook이 덮어쓰는 살아있는
                        #   버퍼라, 그냥 참조를 들면 ci=1 forward 뒤 d[0]과 d[1]이 같은 텐서를
                        #   가리키고 cos이 항등적으로 1이 된다 — 라우팅 붕괴로 오독된다.
                        d[ci] = tap.snapshot()
                    ...
                    if first and cfg.determinism_check:                        # §5-2
                        ...
                        for key in d[0]:
                            assert torch.equal(d[0][key], again[key]), (
                                f"[R9_A] 같은 입력에 Δ가 달라진다 ({key}) — eval 모드/드롭아웃 확인")
                        logging.info(f"[R9_A]   결정론 OK · residual 항등식 {resid_err:.2e} · "
                                     f"최종층 항등식 {fin_err:.2e}")
                    first = False

                    for li in range(n_block):
                        for s in SUBS:
                            # (T=16, B=num_probe, H=512) 두 개. pair_stats가 실행 구간
                            # 토큰만 잘라 (토큰, 프로브)마다 cos을 접고 그 스칼라를 평균한다
                            # — "각도 먼저, 평균 나중"(§2.3). 벡터를 먼저 평균하면 서로 다른
                            # 방향이 상쇄되어 전혀 다른 값이 나온다.
                            d0, d1 = d[0][(li, s)], d[1][(li, s)]
                            c, mm, nok, nall = pair_stats(d0, d1, tok, cfg.norm_floor)
                            if not np.isnan(c):
                                cos_sum[s][si, li, ti] += c
                                cos_cnt[s][si, li, ti] += 1
                            # M은 floor 미만 칸도 포함해 더한다 — cos과 달리 0도 유효한
                            # 크기 정보이고, "기여가 죽었다"는 사실 자체를 재는 양이다.
                            mag_sum[s][si, li, ti] += mm
                            mag_cnt[s][si, li, ti] += 1
                            ok_s[s] += nok
                            all_s[s] += nall
                            # §2.4 영점: 조건을 고정하고 프로브 인덱스만 섞어 각도를 잰다.
                            # c₀·c₁ 양쪽에서 재되, 정의된 것만 세어 평균한다. (칸마다 표본이
                            # 2씩 쌓이지만 shuf_cnt로 나누므로 스케일은 맞는다.)
                            # 이 값이 cos의 비교 기준이다 — 결론은 늘 gap = cos − shuffle로
                            # 읽는다. 층마다 다른 anisotropy를 여기서 상쇄시킨다.
                            for dd in (d0, d1):
                    ...
                    # 버렸나"가 갈린다.
                    #   512 : Δ_512 = x⊙scale(c) + shift(c)  — 블록의 Δ와 같은 의미의 증분
                    #   7   : Δ_7   = W·Δ_512                — 505차원을 버린 뒤 남은 것
                    ...
                    # [0]은 배치 첫 원소. cond를 expand한 것이라 프로브마다 값이 같으므로
                    # 아무거나 집어도 된다.
        ...
        with warnings.catch_warnings():
            ...
            for k in FINALS:
                ...
                logging.info(f"[R9_A]  final{k:>4}: ⟨cos⟩ {np.nanmean(fc):+.3f}  "
                             f"⟨shuffle⟩ {np.nanmean(fs):+.3f}  "
                             f"gap {np.nanmean(fc - fs):+.3f}  "
                             f"⟨‖Δ‖⟩ {np.nanmean(_mean(fmag_sum[k], fpair_cnt)):.3e}")
            logging.info(f"[R9_A]  final mod: ⟨cos([shift|scale](c₀), (c₁))⟩ "
        ...
        with warnings.catch_warnings():
            ...
            for s in SUBS:
                ...
                mag_s = _mean(mag_sum[s], mag_cnt[s])
                logging.info(
                    f"[R9_A]   {s:>4}: ⟨cos⟩ {np.nanmean(cos_s):+.3f}  "
                ...
                if excl[s] > 0.5 or undef[s] > 0.05:
                    logging.warning(colored(
                        f"[R9_A]   {spec['key']}/{s}: 기여가 사실상 소멸한 구간이 있다. "
    ...
    blob["meta"] = np.array(json.dumps({
        ...
        "cl_stage": cfg.cl_stage if cfg.cl_stage >= 0 else cfg.task_b,
    ...
    logging.info(colored(f"[R9_A] saved -> {cache}", "green", attrs=["bold"]))
...
def _draw_timeline(ax, ctx: dict, si: int | None) -> None:
    ...
    ax.set_title(f"m   episode timelines — how long each rollout actually ran "
...
def _draw_magnitude(ctx: dict, out: Path) -> Path:
    ...
    fig.suptitle(
        f"R9_A magnitudes: how big is each sub-block's conditional contribution?   "
        f"task {a} vs task {b}\n"
...
def _draw_final(ctx: dict, si: int | None, out: Path) -> Path:
    """최종 출력단 전용 그림.

    본 그림에서 떼어낸 이유는 자리가 없어서만은 아니다. 이 층은 블록과 성격이 다르다 —
    residual 덧셈이 아니라 x를 통째로 변조하고, 그 뒤 512차원을 7차원으로 사영한다.
    블록 히트맵 옆에 눌러 담으면 두 줄짜리 띠가 되어 읽히지도 않고 옆 패널을 가린다.

    묻는 것은 하나다: 블록에서 살아남은 방향 분리가 **액션 공간까지 남는가**.
        Δ (512-d)  = x⊙scale(c) + shift(c)     조건이 마지막 표현에 더하는 증분
        W·Δ (7-d)  = 그 증분을 readout으로 내린 것 (505차원이 버려진다)
    512에서는 갈렸는데 7에서 정렬로 바뀌면, 라우팅은 살아 있었고 readout이 버린 것이다.
    """
    import matplotlib.pyplot as plt
    ...
    m, keys, titles, short = ctx["m"], ctx["keys"], ctx["titles"], ctx["short"]
    t_grid, obs_steps, nt = ctx["t_grid"], ctx["obs_steps"], ctx["nt"]
    ...
    fig = plt.figure(figsize=(4.3 * n + 2.0, 9.6))
    gs = fig.add_gridspec(2, n + 1, width_ratios=[1] * n + [0.08],
                          height_ratios=[1.0, 1.05], hspace=0.46, wspace=0.24,
                          left=0.075, right=0.945, top=0.80, bottom=0.075)

    # ── a~c: 모델별 스트립 (위 512-d, 아래 7-d) ─────────────────────────────
    for ci, k in enumerate(keys):
        ...
        rgba[..., 3] = 1.0                       # 본 그림과 같이 불투명도 인코딩은 안 쓴다
        ...
        ax.set_yticks([0, 1])
        ax.set_yticklabels(["W·Δ  (7-d)", "Δ  (512-d)"] if ci == 0 else ["", ""], fontsize=9)
        ax.tick_params(colors=INK2, labelsize=8.5, length=3)
        for sp in ax.spines.values():
            sp.set_color(GRID)
        ax.set_xlabel("flow time  t", color=INK2, fontsize=9)
        ax.set_title(f"{'abc'[ci]}   {titles[k]}", fontsize=10.5, color=INK, pad=8, loc="left")
    cax = fig.add_subplot(gs[0, n])
    cb = fig.colorbar(ScalarMappable(norm=norm, cmap=cmap), cax=cax)
    cb.set_label("cos − shuffle", color=INK2, fontsize=8.5, labelpad=2)
    cb.ax.tick_params(colors=INK2, labelsize=7.5)
    cb.ax.text(0.5, 1.02, "aligned\nvs chance", transform=cb.ax.transAxes, ha="center",
               va="bottom", fontsize=6.5, color=DIV_LO, linespacing=1.2)
    cb.ax.text(0.5, -0.02, "orthogonal\nvs chance", transform=cb.ax.transAxes, ha="center",
               va="top", fontsize=6.5, color=DIV_HI, linespacing=1.2)
    cb.outline.set_edgecolor(GRID)

    # ── d~g: flow time별 cos(실선)+shuffle(점선), 그리고 물리시간별 gap ──────
    row = gs[1, :].subgridspec(1, 4, wspace=0.34)
    for i, w in enumerate(FINALS):
        ...
        ax.set_title(f"{'de'[i]}   {FINAL_TITLE[w]}", fontsize=10, color=INK, pad=8, loc="left")
    # 물리시간별 gap — 평균 하나로는 알 수 없는 "스텝마다 일관된가"를 보는 패널
    for i, w in enumerate(FINALS):
        ax = fig.add_subplot(row[0, 2 + i])
        _shade_dead(ax, obs_steps, ctx["dead_frac"])
        for ki, k in enumerate(keys):
            g = _nanmean(ctx["fcos_s"][(k, w)] - ctx["fshuf_s"][(k, w)], axis=1)
            ax.plot(obs_steps, g, color=MODEL_COLORS[k], lw=2.0, marker="o", ms=4, zorder=4)
            j = max(0, len(obs_steps) - 1 - ki)
            if np.isfinite(g[j]):
                ax.annotate(short[k], (obs_steps[j], g[j]), textcoords="offset points",
                            xytext=(0, 7), fontsize=8, color=MODEL_COLORS[k],
                            fontweight="bold", ha="center")
        ax.axhline(0.0, color=INK2, lw=0.9, ls=":", zorder=2)
        if not summary:
            ax.axvline(float(obs_steps[si]), color=DIV_LO, lw=1.6, alpha=0.85, zorder=6)
            ax.text(float(obs_steps[si]), 1.0, f" step {int(obs_steps[si])} ",
                    transform=ax.get_xaxis_transform(), fontsize=7.5, color=DIV_LO,
                    ha="left", va="top", fontweight="bold")
        _style(ax)
        ax.grid(True, color=GRID, lw=0.5, alpha=0.7)
        ax.set_xticks(obs_steps)
        ax.set_xlabel("rollout step  (physical time)", color=INK2, fontsize=9)
        ax.set_ylabel(f"gap  ({'512-d' if w == '512' else '7-d'})", color=INK2, fontsize=9)
        ax.set_title(f"{'fg'[i]}   gap over physical time — {'512-d' if w == '512' else '7-d'}",
                     fontsize=10, color=INK, pad=8, loc="left")

    handles = [Line2D([0], [0], color=MODEL_COLORS[k], lw=2.4, marker="o", ms=5,
                      label=titles[k]) for k in keys]
    handles += [Line2D([0], [0], color=INK2, lw=1.4, ls="--", label="shuffle baseline")]
    fig.legend(handles=handles, loc="lower center", ncol=len(handles), frameon=False,
               fontsize=9, labelcolor=INK2, bbox_to_anchor=(0.5, 0.008))
    f512 = {k: float(_nanmean(d["fgap"][(k, "512")])) for k in keys}
    f7 = {k: float(_nanmean(d["fgap"][(k, "7")])) for k in keys}
    when = ("averaged over all rollout steps" if summary
            else f"rollout step {int(obs_steps[si])} only "
                 f"({si + 1} of {len(obs_steps)}, {int(ctx['n_live'][si])}/{ctx['n_ep']} "
                 f"episodes live)")
    fig.suptitle(
        f"R9_A · final layer — does the routing survive the 512→7 readout?      "
        f"conditions: task {a} vs task {b}      {when}\n"
        f"Δ = x⊙scale(c) + shift(c) is what the conditioning adds to the last hidden "
        f"representation; W·Δ is the part that reaches action space (505 of 512 dims are "
        f"discarded).\n"
        f"Colour and scale are shared with the block map, so the two figures can be read "
        f"together. Solid = cos between the two conditions, dashed = its shuffle baseline.\n"
        f"gap Δ(512-d) → W·Δ(7-d):   " +
        " · ".join(f"{short[k]} {f512[k]:+.3f} → {f7[k]:+.3f}" for k in keys) +
        f"\nA sign flip between the two rows means the direction was still separated at "
        f"512-d and the readout dropped it — panels f/g show whether that holds step by step.",
        fontsize=10.5, color=INK, y=0.985, linespacing=1.45)
    fig.savefig(out, dpi=170, facecolor="white")
    if summary:
        fig.savefig(out.with_suffix(".pdf"), facecolor="white")
    plt.close(fig)
...
def _draw(ctx: dict, si: int | None, out: Path) -> Path:
    ...
    if summary_fig:
        scope = (f"observations at rollout steps 0–{int(obs_steps[-1])} every "
                 f"{m['obs_stride']} ({ns} points) × {m['num_obs']} episodes; "
                 f"heat maps average over them, panels k/l keep the axis")
        rule = "averaged over t and rollout step"
    else:
        dead = ctx["dead_frac"][si]
        warn = ("" if dead <= 0 else
                f"  {dead * 100:.0f}% of episodes had already ended here")
        scope = (f"observation at rollout step {int(obs_steps[si])} only "
                 f"({si + 1} of {ns}) × {m['num_obs']} episodes{warn}; "
                 f"colour scale shared with every other step figure")
        rule = "averaged over t"
    stage = m.get("cl_stage", m["task_b"])
    n_undef = sum(int(np.sum(~np.isfinite(cos[ks]))) for ks in cos)
    grey = (f"  {n_undef} cells grey (‖Δ‖ below {m['norm_floor']:.0e}, cos undefined — "
            f"never filled with 0)." if n_undef else "")
    # ★ 캡션은 짧게. 정의·좌표계·해시 같은 상세는 method.md가 들고 있고, 여기서는
    #   "무엇을 그린 그림인지"와 "자동으로 센 판정"만 남긴다. 길어지면 본문 패널을
    #   밀어내 서로 가린다.
    caption = (
        f"R9_A — conditional routing map      "
        f"CL (task 0 → … → {stage})  vs  joint (all tasks mixed)  vs  pretrained      "
        f"conditions: task {a} vs task {b}\n"
        f"Colour = cos(Δ(c₀), Δ(c₁)) − that cell's shuffle baseline, so white is chance:  "
        f"blue = more orthogonal than chance (routing) · red = more aligned.\n"
        f"Δ is what each sub-block adds to the residual stream, α(c) ⊙ sub-block output. "
        f"No opacity encoding; magnitudes are in <name>_magnitude.png.{grey}\n"
        f"Probes: x_t = (1−t)·x₀ + t·a, {m['num_probe']} points; {scope}.\n"
        f"Routing blocks — (cos − shuffle) {rule} below −{gth:g} and ‖Δ‖ ≥ {mfrac:g}× the "
        f"global max:   {route_txt}   (of {L} each).\n"
        f"The 512→7 readout the block hooks never see lives in <name>_final.png:  "
        f"gap Δ(512-d) → W·Δ(7-d)   " +
        " · ".join(f"{short[k]} {f512[k]:+.2f}→{f7[k]:+.2f}" for k in keys) +
        f".\nClaim is limited to where the directional separation changes; it does not "
        f"explain forgetting."
    )
    # 캡션 줄 수가 그림마다 다르므로 위 여백을 줄 수에서 역산한다(고정하면 긴 캡션이
    # 패널 제목을 덮는다). 크기 패널을 별도 그림으로 뺀 뒤라 본체는 5행이다.
    fig_h = 15.4 if summary_fig else 12.6
    ...
    nrow = 4 if summary_fig else 3
    ratios = ([1.0, 1.0, 0.66, 0.62] if summary_fig else [1.0, 1.0, 0.62])
    ...
    gs = fig.add_gridspec(nrow, n + 2, width_ratios=[1] * n + [0.075, 1.05],
                          height_ratios=ratios, hspace=0.62, wspace=0.24,
    ...
    cb.set_label("cos − shuffle", color=INK2, fontsize=8, labelpad=1)
    cb.ax.tick_params(colors=INK2, labelsize=7.5)
    ...
    for ri, s in enumerate(SUBS):
        ...
        ax.set_xlabel(f"cos  ({s})", color=INK2, fontsize=8.5)
    ...
    for i, s in enumerate(SUBS):
        ...
        ax.set_xlim(-0.02, 1.08)
        ax.set_xlabel("flow time  t", color=INK2, fontsize=8.5)
        ax.set_ylabel(f"cos  ({s})", color=INK2, fontsize=8.5)
...
def plot_r9a(cache: str | Path, out_png: str | Path | None = None,
    ...
    fn_ = _draw_final(ctx, None, out.with_name(out.stem + "_final.png"))
    print(f"saved figure -> {fn_}  (+ .pdf)   최종 출력단 전용")
    if not per_step:
        return
    ...
    for si, step in enumerate(ctx["obs_steps"]):
        ...
        _draw_final(ctx, si, Path(f"{stem}_final_step{int(step):04d}.png"))
        print(f"saved figure -> {p}  (+ _final_step)   "
              f"({int(ctx['n_live'][si])}/{ctx['n_ep']} episodes live)")
...
def main(cfg: R9AConfig):
    ...
    if cache.exists() and not cfg.recompute:
        logging.info(f"[R9_A] 캐시 재사용: {cache}")
    else:
        cache = run_probe(cfg, run_dir)
    if not cfg.no_plot:
        plot_r9a(cache, per_step=cfg.per_step_figs)
...
if __name__ == "__main__":
    ...
    if "--plot_only" in sys.argv:
        ...
        if "cache" in kv:
            plot_r9a(kv["cache"], per_step=per_step)
        elif "run_dir" in kv:
            found = sorted(Path(kv["run_dir"]).glob("R9A_*.npz"))
            if not found:
                raise SystemExit(f"[R9_A] npz 캐시가 없다: {kv['run_dir']}")
            for c in found:
                plot_r9a(c, per_step=per_step)
        else:
