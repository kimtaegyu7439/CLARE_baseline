#!/usr/bin/env python
"""R15 — R11 에서 앵커 좌표만 가우시안 샘플링으로 바꾼 것.

  R11   z = (o − mu_new[τ]) / sigma_new[τ]   현재 관측의 실제 편차 (수송)
  R15   z ~ N(0, I)                           매 스텝 새 난수 (샘플링)
  공통    b_j = mu_j[τ] + sigma_j[τ] · z

R12/R13 이 level 앵커만으로 이 축을 봤다면, R15 은 structure 항이 있는
R11 위에서 같은 축을 본다. 네 팔이 2x2 를 이룬다.
...
OUT_DIR = REPO / "results" / "R15"


class R15Anchor(R11.R11Anchor):
    """R11 + 샘플링."""

    name = "R15"
    sample_z = True          # R11 과의 유일한 차이

    def describe(self):
        return "R15 — " + super().describe().split("—", 1)[1].strip() + " + 가우시안 샘플 좌표"
...
def main() -> None:
    ...
    B1.ANCHOR = R15Anchor(args)

    argv = ["B1.py", "--p_drop", "0", "--guidance_w", "1.0", "--lambda_anchor", "1.0",
            "--out_dir", str(out_dir),
            "--ckpt_root", str(REPO / "outputs" / "R15")]
    if args.smoke:
        argv.append("--smoke")
    ...
    json.dump({"arm": "R15", "base": "R11", "anchor_coord": "gaussian sample",
              ...
              (out_dir / "r15_config.json").open("w"), indent=2, ensure_ascii=False)
    ...
    try:
        ...
        sys.argv = old
    R10.write_table(out_dir, arm="R15", subtitle="R11 + 샘플링 (가우시안 샘플 좌표)")
